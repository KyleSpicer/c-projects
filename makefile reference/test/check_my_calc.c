...
START_TEST(test_add)
{
	ck_assert_int_eq(1, add(0, 1));
	ck_assert_int_eq(10, add(6, 4));
}
END_TEST

START_TEST(test_sub)
{
	ck_assert_int_eq(5, sub(6, 1));
	ck_assert_int_eq(2, sub(6, 4));
}
END_TEST

Suite *check_calc_suite(void)
{
	Suite *suite;
	TCase *tc_core;
